...
import { CalendarioLaboral, UpdateCalendarioDTO, REGIONES_ESPANA, Festivo, TIPOS_FESTIVO } from '@/types/calendario.types'
...
import { ArrowLeft, Save, CalendarDays, RefreshCw, Plus, Trash2, Download } from 'lucide-react'
...
export default function EditarCalendarioPage() {
  ...
  const [formData, setFormData] = useState<UpdateCalendarioDTO & { anio: number }>({
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            nombre: cal.nombre,
            region: cal.region || '',
            provincia: cal.provincia || '',
            localidad: cal.localidad || '',
            esDefecto: cal.esDefecto,
            activo: cal.activo,
          })
          setFestivos(cal.festivos || [])
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      const response = await calendariosService.update(params.id as string, { ...formData, festivos })
      if (response.success) {
        toast.success('Calendario actualizado correctamente')
        router.push(`/calendarios/${params.id}`)
      }
    } catch (error: any) {
      ...
      <div className="w-full max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href={`/calendarios/${params.id}`}><ArrowLeft className="h-5 w-5" /></Link>
          </Button>
          <div>
            ...
            <p className="text-sm text-muted-foreground">Modifica los datos del calendario</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="bg-muted"
                  />
                  <p className="text-xs text-muted-foreground">El año no se puede modificar</p>
                </div>
                <div className="space-y-2">
          ...
          <div className="flex justify-end gap-3 mt-6">
            <Button type="button" variant="outline" asChild>
              <Link href={`/calendarios/${params.id}`}>Cancelar</Link>
            </Button>
