...
interface DepartamentosFilters {
...
const DEFAULT_CONFIG = {
  columnas: [
    ...
    { key: 'responsableNombre', visible: true, orden: 3 },
    { key: 'color', visible: true, orden: 4 },
...
export default function DepartamentosPage() {
  ...
  const [filters, setFilters] = useState<DepartamentosFilters>({ page: 1, limit: 25, sortBy: 'orden', sortOrder: 'asc', activo: 'true' })
  ...
  const columnasDisponibles = [
    ...
    { key: 'responsableNombre', label: 'Responsable', sortable: true },
    { key: 'color', label: 'Color', sortable: false },
    { key: 'activo', label: 'Estado', sortable: true },
  ]

  const { config: moduleConfig, isLoading: isLoadingConfig, updateColumnas, updateSortConfig, updateDensidad, resetConfig } = useModuleConfig('departamentos', DEFAULT_CONFIG, { autoSave: true, debounceMs: 1000 })
  ...
  }), [departamentos, pagination?.total])
  ...
  const cargarDepartamentos = useCallback(async () => {
    try {
      ...
      const response = await departamentosService.getAll(params)
      if (response.success) {
  ...
  const handleGuardarVista = useCallback(async (nombre: string, descripcion?: string, esDefault?: boolean, vistaId?: string) => {
    try {
      if (vistaId) {
        await vistasService.update(vistaId, { modulo: 'departamentos', nombre, descripcion, configuracion: moduleConfig, esDefault: esDefault || false })
        toast.success(`Vista "${nombre}" actualizada`)
      } else {
        await vistasService.create({ modulo: 'departamentos', nombre, descripcion, configuracion: moduleConfig, esDefault: esDefault || false })
  ...
  useEffect(() => {
    const cargarVistaDefault = async () => {
      try {
        const vistas = await vistasService.getAll('departamentos', true)
  ...
  const handleSelectAll = () => { if (selectAll) setSelectedItems([]); else setSelectedItems(departamentos.map(d => d._id)); setSelectAll(!selectAll) }
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      toast.success('Departamento(s) eliminado(s)');
      cargarDepartamentos();
      setSelectedItems([]);
      setSelectAll(false);
  ...
  const handleAction = async (id: string, action: string) => {
    switch (action) {
      case 'view': router.push(`/departamentos/${id}`); break
      case 'edit': router.push(`/departamentos/${id}/editar`); break
      case 'delete': const d = departamentos.find(dp => dp._id === id); if (d) setDeleteDialog({ open: true, ids: [id], nombres: [d.nombre] }); break
    }
  }

  const handleBulkAction = (action: string) => {
    if (action === 'delete' && selectedItems.length > 0) {
      const nombres = departamentos.filter(d => selectedItems.includes(d._id)).map(d => d.nombre)
    ...
    <DashboardLayout>
      <div className="w-full space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2"><Building2 className="h-7 w-7 text-primary" />Departamentos</h1>
            <p className="text-sm text-muted-foreground mt-1">Gestion de departamentos de la empresa</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" size="sm" onClick={cargarDepartamentos}><RefreshCw className="h-4 w-4" /><span className="ml-2 hidden sm:inline">Actualizar</span></Button>
            {canCreate('departamentos') && (
              <Button asChild size="sm"><Link href="/departamentos/nuevo"><Plus className="h-4 w-4 mr-2" /><span className="hidden sm:inline">Nuevo Departamento</span></Link></Button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            <SettingsMenu densidad={densidad} onDensidadChange={(d) => { updateDensidad(d); toast.success(`Densidad: ${d}`) }} modulo="departamentos" configuracionActual={moduleConfig} onAplicarVista={handleAplicarVista} onGuardarVista={handleGuardarVista} onRestablecer={async () => { await resetConfig(); toast.success('Configuracion restablecida') }} />
            ...
            <ExportButton data={departamentos} columns={columnasDisponibles.filter(c => columnasVisibles.includes(c.key)).map(c => ({ key: c.key, label: c.label }))} filename="departamentos" stats={[{ label: 'Total', value: stats.total }, { label: 'Activos', value: stats.activos }]} />
            <PrintButton data={departamentos} columns={columnasDisponibles.filter(c => columnasVisibles.includes(c.key)).map(c => ({ key: c.key, label: c.label }))} title="Listado de Departamentos" stats={[{ label: 'Total', value: stats.total }, { label: 'Activos', value: stats.activos }]} filters={columnFiltersInput} />
          </div>
        </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  {columnasVisibles.includes('descripcion') && <th className={`${densityClasses.header} text-left w-[250px]`}><span className="text-xs font-semibold uppercase tracking-wider">Descripcion</span></th>}
                  {columnasVisibles.includes('responsableNombre') && <th className={`${densityClasses.header} text-left w-[180px]`}><button onClick={() => handleSort('responsableNombre')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider">Responsable{getSortIcon('responsableNombre')}</button></th>}
                  {columnasVisibles.includes('color') && <th className={`${densityClasses.header} text-left w-[80px]`}><span className="text-xs font-semibold uppercase tracking-wider">Color</span></th>}
                ...
                <tr className="border-b bg-card">
                  ...
                  {columnasVisibles.includes('descripcion') && <th className="px-3 py-1.5"></th>}
                  {columnasVisibles.includes('responsableNombre') && <th className="px-3 py-1.5"></th>}
              ...
              <tbody className="divide-y">
                {isLoading ? (
                  ...
                  <tr><td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center"><Building2 className="h-12 w-12 mx-auto mb-2 opacity-20" /><p className="font-medium">No se encontraron departamentos</p></td></tr>
                ) : departamentos.map((dept) => (
                  <tr key={dept._id} className="hover:bg-muted/30 transition-colors group cursor-pointer" onClick={() => router.push(`/departamentos/${dept._id}`)}>
                    <td className={`${densityClasses.cell} sticky left-0 z-20 bg-card group-hover:bg-muted/50`} onClick={(e) => e.stopPropagation()}><Checkbox checked={selectedItems.includes(dept._id)} onCheckedChange={() => handleSelectItem(dept._id)} /></td>
                    {columnasVisibles.includes('codigo') && <td className={`${densityClasses.cell} font-mono font-medium`}>{dept.codigo}</td>}
                    {columnasVisibles.includes('nombre') && <td className={`${densityClasses.cell} font-medium`}>{dept.nombre}</td>}
                    {columnasVisibles.includes('descripcion') && <td className={`${densityClasses.cell} text-muted-foreground`}>{dept.descripcion || '-'}</td>}
                    {columnasVisibles.includes('responsableNombre') && <td className={`${densityClasses.cell}`}>{dept.responsableNombre || '-'}</td>}
                    {columnasVisibles.includes('color') && <td className={`${densityClasses.cell}`}><div className="w-6 h-6 rounded-full border" style={{ backgroundColor: dept.color || '#3B82F6' }} /></td>}
                    {columnasVisibles.includes('activo') && <td className={`${densityClasses.cell}`}><Badge variant={dept.activo ? 'default' : 'secondary'} className={dept.activo ? 'bg-green-100 text-green-800' : ''}>{dept.activo ? 'Activo' : 'Inactivo'}</Badge></td>}
                    <td className={`${densityClasses.cell} sticky right-0 z-20 bg-card group-hover:bg-muted/50 text-right`} onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56">
                          ...
                          <DropdownMenuItem onClick={() => handleAction(dept._id, 'view')}><Eye className="mr-2 h-4 w-4" />Ver detalle</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleAction(dept._id, 'edit')}><Edit className="mr-2 h-4 w-4" />Editar</DropdownMenuItem>
                          {canDelete('departamentos') && (
                            <>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-destructive" onClick={() => handleAction(dept._id, 'delete')}><Trash2 className="mr-2 h-4 w-4" />Eliminar</DropdownMenuItem>
                            </>
                          )}
                        </DropdownMenuContent>
                      </DropdownMenu>
        ...
        <Dialog open={deleteDialog.open} onOpenChange={(o) => setDeleteDialog({ ...deleteDialog, open: o })}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>¿Eliminar {deleteDialog.ids.length === 1 ? 'el departamento' : `${deleteDialog.ids.length} departamentos`}?</DialogDescription>
