...
import { ArrowLeft, Tag, Edit, RefreshCw, Trash2 } from 'lucide-react'
...
export default function VerEstadoPage() {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || 'Error al eliminar el estado')
    } finally {
      setIsDeleting(false)
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Detalles del estado: {estado?.nombre}
            </p>
          </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción eliminará permanentemente el estado "{estado?.nombre}".
