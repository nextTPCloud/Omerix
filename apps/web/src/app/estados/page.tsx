...
export default function EstadosPage() {
  ...
  // Filtros generales
  const [filters, setFilters] = useState<EstadosFilters>({
    page: 1,
    limit: 25,
    sortBy: 'nombre',
    sortOrder: 'asc',
  ...
  const {
    ...
    updateDensidad,
    resetConfig,
  } = useModuleConfig('estados', DEFAULT_ESTADOS_CONFIG, {
    autoSave: true,
    debounceMs: 1000,
  })
  ...
  const sortConfig = useMemo(() => {
    return moduleConfig?.sortConfig || DEFAULT_ESTADOS_CONFIG.sortConfig
  }, [moduleConfig])
  ...
  const stats = useMemo(() => {
    ...
    const inactivos = estados.filter((e) => !e?.activo).length
  ...
  const cargarEstados = useCallback(async () => {
    try {
      ...
      const response = await estadosService.getAll(params)
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedEstados(estados.map(e => e._id))
    }
    setSelectAll(!selectAll)
  }

  const handleSelectEstado = (estadoId: string) => {
    if (selectedEstados.includes(estadoId)) {
      setSelectedEstados(selectedEstados.filter(id => id !== estadoId))
    } else {
      setSelectedEstados([...selectedEstados, estadoId])
    }
  }
  ...
  const handleBulkAction = (action: string) => {
    switch (action) {
      case 'export':
        toast.success('Estados exportados correctamente')
        break
      case 'delete':
        if (selectedEstados.length > 0) {
          const nombresSeleccionados = estados
            .filter(e => selectedEstados.includes(e._id))
            .map(e => e.nombre)

          setDeleteDialog({
            open: true,
            estadoIds: selectedEstados,
            estadoNombres: nombresSeleccionados,
          })
        }
        break
      case 'activate':
    ...
    try {
      await Promise.all(
        selectedEstados.map(id =>
          estadosService.update(id, { activo: activate })
        )
      )
      toast.success(`Estados ${activate ? 'activados' : 'desactivados'} correctamente`)
      cargarEstados()
      setSelectedEstados([])
      setSelectAll(false)
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await Promise.all(
        deleteDialog.estadoIds.map(id => estadosService.delete(id))
      )

      toast.success('Estado(s) eliminado(s) correctamente')
      cargarEstados()
      setSelectedEstados([])
      setSelectAll(false)
      setDeleteDialog({ open: false, estadoIds: [], estadoNombres: [] })
    } catch (error) {
  ...
  const handleEstadoAction = async (estadoId: string, action: string) => {
    switch (action) {
      case 'view':
        router.push(`/estados/${estadoId}`)
        break
      case 'edit':
        router.push(`/estados/${estadoId}/editar`)
        break
      case 'duplicate':
        try {
          toast.loading('Duplicando estado...')
          const response = await estadosService.duplicar(estadoId)
          toast.dismiss()
          if (response.success) {
            toast.success('Estado duplicado correctamente')
            router.push(`/estados/${response.data._id}/editar`)
          }
        } catch (error: any) {
          toast.dismiss()
          toast.error(error.response?.data?.message || 'Error al duplicar el estado')
        }
        break
      case 'delete':
        const estado = estados.find(e => e._id === estadoId)
        if (estado) {
          setDeleteDialog({
            open: true,
            estadoIds: [estadoId],
            estadoNombres: [estado.nombre],
          })
        }
        break
      default:
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Gestiona los estados de tu sistema
            </p>
          </div>
          ...
          <Card className="p-3 bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
            <div className="flex flex-wrap gap-2 items-center">
              <span className="text-sm font-medium text-blue-900 dark:text-blue-100">
                {selectedEstados.length} {selectedEstados.length === 1 ? 'estado seleccionado' : 'estados seleccionados'}
              </span>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                      ...
                      <p className="font-medium">No se encontraron estados</p>
                      <p className="text-xs mt-1">Prueba ajustando los filtros o crear un nuevo estado</p>
                    </td>
                  </tr>
                ) : (
                  estados.map((estado) => (
                    ...
                    >
                      <td className={`${densityClasses.cell} sticky left-0 z-20 bg-card group-hover:bg-muted/50 transition-colors`}>
                        <div className="flex items-center justify-center">
                          <Checkbox
                            checked={selectedEstados.includes(estado._id)}
                            onCheckedChange={() => handleSelectEstado(estado._id)}
                          />
                        </div>
                      </td>
                      ...
                      <td className={`${densityClasses.cell} sticky right-0 z-20 bg-card group-hover:bg-muted/50 transition-colors text-right`}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-56">
                            ...
                            <DropdownMenuItem onClick={() => handleEstadoAction(estado._id, 'view')}>
                              <Eye className="mr-2 h-4 w-4" />
                              Ver detalle
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleEstadoAction(estado._id, 'edit')}>
                              <Edit className="mr-2 h-4 w-4" />
                              Editar
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleEstadoAction(estado._id, 'duplicate')}>
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() => handleEstadoAction(estado._id, 'delete')}
                            >
                              <Trash2 className="mr-2 h-4 w-4" />
                              Eliminar
                            </DropdownMenuItem>
        ...
        <Dialog open={deleteDialog.open} onOpenChange={(open) =>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ¿Estás seguro de que deseas eliminar {deleteDialog.estadoIds.length === 1
                  ? 'el siguiente estado'
                  : `los siguientes ${deleteDialog.estadoIds.length} estados`}?
                <ul className="mt-3 max-h-32 overflow-y-auto space-y-1">
                  {deleteDialog.estadoNombres.map((nombre, index) => (
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setDeleteDialog({ open: false, estadoIds: [], estadoNombres: [] })}
              >
                Cancelar
              </Button>
              <Button
