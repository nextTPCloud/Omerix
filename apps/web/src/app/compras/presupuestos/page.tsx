...
const DEFAULT_CONFIG = {
  columnas: [
    ...
    { key: 'fechaValidez', visible: true, orden: 6 },
    { key: 'totalPresupuesto', visible: true, orden: 7 },
  ] as ColumnaConfig[],
  sortConfig: {
    key: 'fechaCreacion',
...
export default function PresupuestosCompraPage() {
  ...
  // Estados de datos
  const [presupuestos, setPresupuestos] = useState<PresupuestoCompra[]>([])
  ...
  // Filtros generales
  const [filters, setFilters] = useState<PresupuestosCompraFilters>({
    page: 1,
    limit: 25,
  ...
  const [columnasDisponibles] = useState([
    ...
    { key: 'fechaValidez', label: 'Validez', sortable: true },
  ...
  const {
    ...
    updateDensidad,
    resetConfig,
  } = useModuleConfig('presupuestos-compra', DEFAULT_CONFIG, {
    autoSave: true,
    debounceMs: 1000,
  })
  ...
  const stats = useMemo(() => {
    ...
    const recibidos = presupuestos.filter((p) => p?.estado === 'recibido').length
    ...
    const expirados = presupuestos.filter((p) => p?.estado === 'expirado').length
  ...
  const cargarPresupuestos = useCallback(async () => {
    try {
      setIsLoading(true)
      const response = await presupuestosCompraService.getAll(filters)
      ...
      if (response.success) {
        ...
        setPresupuestos([])
        toast.error('Error al cargar los presupuestos de compra')
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    Object.entries(debouncedColumnFilters).forEach(([key, value]) => {
      if (key === 'estado') {
        ...
        if (value) combinedFilters.fechaDesde = value
      } else if (key === 'fechaHasta') {
        if (value) combinedFilters.fechaHasta = value
      } else if (key === 'importeMinimo') {
        if (value) combinedFilters.importeMinimo = parseFloat(value)
      }
    })
  ...
  const handleSelectPresupuesto = (id: string) => {
    if (selectedPresupuestos.includes(id)) {
      setSelectedPresupuestos(selectedPresupuestos.filter(i => i !== id))
    } else {
      setSelectedPresupuestos([...selectedPresupuestos, id])
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    try {
      if (deleteDialog.presupuestoIds.length === 1) {
        await presupuestosCompraService.delete(deleteDialog.presupuestoIds[0])
      } else {
        await presupuestosCompraService.deleteMany(deleteDialog.presupuestoIds)
      }

      toast.success('Presupuesto(s) de compra eliminado(s) correctamente')
      cargarPresupuestos()
      setSelectedPresupuestos([])
      setSelectAll(false)
  ...
  const handlePresupuestoAction = async (id: string, action: string) => {
    switch (action) {
      case 'view':
        router.push(`/compras/presupuestos/${id}`)
        break
      case 'edit':
        router.push(`/compras/presupuestos/${id}/editar`)
        break
      case 'duplicate':
        try {
          toast.loading('Duplicando presupuesto...')
          const response = await presupuestosCompraService.duplicar(id)
          toast.dismiss()
      ...
      case 'recibido':
        try {
          await presupuestosCompraService.cambiarEstado(id, 'recibido')
          toast.success('Presupuesto marcado como recibido')
          cargarPresupuestos()
        } catch (error) {
      ...
      case 'aceptar':
        try {
          await presupuestosCompraService.cambiarEstado(id, 'aceptado')
      ...
      case 'rechazar':
        try {
          await presupuestosCompraService.cambiarEstado(id, 'rechazado')
      ...
      case 'convertir':
        try {
          ...
          const response = await presupuestosCompraService.convertirAPedido(id)
          toast.dismiss()
          if (response.success) {
      ...
      case 'cancelar':
        try {
          await presupuestosCompraService.cambiarEstado(id, 'cancelado')
  ...
  const getDiasValidez = (fecha: any) => {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Gestiona los presupuestos recibidos de proveedores
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={cargarPresupuestos}>
              <RefreshCw className="h-4 w-4" />
              <span className="ml-2 hidden sm:inline">Actualizar</span>
            </Button>
{canCreate('presupuestos-compra') && (
            <Button asChild size="sm">
              <Link href="/compras/presupuestos/nuevo">
            ...
            <Card className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{stats.convertidos}</p>
                </div>
              </div>
            </Card>
            <Card className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{stats.expirados}</p>
                </div>
              </div>
            </Card>
            <Card className="p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            ...
            <ExportButton
              ...
              filename="presupuestos-compra"
              stats={[
                { label: 'Total', value: stats.total },
                { label: 'Aceptados', value: stats.aceptados },
                { label: 'Convertidos', value: stats.convertidos },
            ...
            <PrintButton
              ...
              columns={columnasDisponibles
                .filter((col) => columnasVisibles.includes(col.key))
                .map((col) => ({ key: col.key, label: col.label }))}
              title="Listado de Presupuestos de Compra"
              stats={[
                { label: 'Total', value: stats.total },
                { label: 'Aceptados', value: stats.aceptados },
                { label: 'Convertidos', value: stats.convertidos },
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  {columnasVisibles.includes('proveedorNombre') && (
                    <th className={`${densityClasses.header} text-left w-[200px]`}>
                      <button onClick={() => handleSort('proveedorNombre')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider transition-colors">
                        Proveedor {getSortIcon('proveedorNombre')}
                      </button>
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('titulo') && (
                    <th className={`${densityClasses.header} text-left w-[220px]`}>
                      <button onClick={() => handleSort('titulo')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider transition-colors">
                        Titulo {getSortIcon('titulo')}
                      </button>
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('prioridad') && (
                    <th className={`${densityClasses.header} text-left min-w-[100px]`}>
                      <button onClick={() => handleSort('prioridad')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider transition-colors">
                  ...
                  {columnasVisibles.includes('fechaValidez') && (
                    <th className={`${densityClasses.header} text-left min-w-[130px]`}>
                      <button onClick={() => handleSort('fechaValidez')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider transition-colors">
                        Validez {getSortIcon('fechaValidez')}
                      </button>
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('totalPresupuesto') && (
                    <th className={`${densityClasses.header} text-right min-w-[130px]`}>
                      <button onClick={() => handleSort('totales.totalPresupuesto')} className="flex items-center justify-end w-full hover:text-primary text-xs font-semibold uppercase tracking-wider transition-colors">
                        Importe Total {getSortIcon('totales.totalPresupuesto')}
                      </button>
                    </th>
                  )}
                ...
                <tr className="border-b bg-background">
                  ...
                  {columnasVisibles.includes('proveedorNombre') && (
                    <th className="px-3 py-1.5">
                      <Input placeholder="Filtrar..." className="h-7 text-xs" value={columnFiltersInput.proveedorNombre || ''} onChange={(e) => handleColumnFilterInput('proveedorNombre', e.target.value)} />
                    </th>
                  )}

                  {columnasVisibles.includes('titulo') && (
                    <th className="px-3 py-1.5">
                      <Input placeholder="Filtrar..." className="h-7 text-xs" value={columnFiltersInput.titulo || ''} onChange={(e) => handleColumnFilterInput('titulo', e.target.value)} />
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('estado') && (
                    <th className="px-3 py-1.5">
                      <TableSelect
                        ...
                        placeholder="Todos"
                        options={[{ value: 'all', label: 'Todos' }, ...ESTADOS_PRESUPUESTO_COMPRA.map(e => ({ value: e.value, label: e.label }))]}
                      />
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('prioridad') && (
                    <th className="px-3 py-1.5">
                      <TableSelect
                        ...
                        placeholder="Todas"
                        options={[{ value: 'all', label: 'Todas' }, ...PRIORIDADES.map(p => ({ value: p.value, label: p.label }))]}
                      />
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('fecha') && (
                    <th className="px-3 py-1.5">
                      <Input type="date" className="h-7 text-xs" value={columnFiltersInput.fechaDesde || ''} onChange={(e) => handleColumnFilterInput('fechaDesde', e.target.value)} title="Fecha desde" />
                    </th>
                  )}

                  {columnasVisibles.includes('fechaValidez') && (
                    <th className="px-3 py-1.5">
                      <Input type="date" className="h-7 text-xs" value={columnFiltersInput.fechaHasta || ''} onChange={(e) => handleColumnFilterInput('fechaHasta', e.target.value)} title="Validez hasta" />
                    </th>
                  )}

                  {columnasVisibles.includes('totalPresupuesto') && (
                    <th className="px-3 py-1.5">
                      <Input placeholder="Min..." className="h-7 text-xs" type="number" value={columnFiltersInput.importeMinimo || ''} onChange={(e) => handleColumnFilterInput('importeMinimo', e.target.value)} />
                    </th>
                  )}
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                      ...
                      Cargando presupuestos de compra...
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                      ...
                      <p className="font-medium">No se encontraron presupuestos de compra</p>
                      <p className="text-xs mt-1">Prueba ajustando los filtros o crear uno nuevo</p>
                    </td>
                  </tr>
                ) : (
                  presupuestos.map((presupuesto) => {
                    const diasValidez = getDiasValidez(presupuesto.fechaValidez)
                    const estadoConfig = getEstadoConfig(presupuesto.estado)
                    const prioridadConfig = getPrioridadConfig(presupuesto.prioridad)

                    return (
                      <tr key={presupuesto._id} className="hover:bg-muted/30 transition-colors group">
                        <td className={`${densityClasses.cell} sticky left-0 z-20 bg-background group-hover:bg-muted/30 transition-colors`}>
                          <div className="flex items-center justify-center">
                            <Checkbox checked={selectedPresupuestos.includes(presupuesto._id)} onCheckedChange={() => handleSelectPresupuesto(presupuesto._id)} />
                          </div>
                        </td>

                        {columnasVisibles.includes('codigo') && (
                          <td className={`${densityClasses.cell} font-mono ${densityClasses.text} font-medium`}>
                            <Link href={`/compras/presupuestos/${presupuesto._id}`} className="hover:underline hover:text-primary">
                              {presupuesto.codigo}
                            </Link>
                          </td>
                        )}
                        ...
                        {columnasVisibles.includes('proveedorNombre') && (
                          <td className={`${densityClasses.cell} ${densityClasses.text}`}>
                            <div className="max-w-[200px] truncate" title={getProveedorNombre(presupuesto.proveedorId, presupuesto.proveedorNombre)}>
                              {getProveedorNombre(presupuesto.proveedorId, presupuesto.proveedorNombre)}
                            </div>
                          </td>
                        )}
                        ...
                        {columnasVisibles.includes('fechaValidez') && (
                          <td className={`${densityClasses.cell} ${densityClasses.text}`}>
                            <div className="flex items-center gap-1">
                              ...
                              {diasValidez !== null && presupuesto.estado !== 'convertido' && presupuesto.estado !== 'cancelado' && presupuesto.estado !== 'rechazado' && (
                            ...
                            <DropdownMenuContent align="end" className="w-56">
                              ...
                              {presupuesto.estado === 'borrador' && (
                                <DropdownMenuItem onClick={() => handlePresupuestoAction(presupuesto._id, 'enviar')}>
                                  <Send className="mr-2 h-4 w-4 text-blue-600" /> Enviar Solicitud
                                </DropdownMenuItem>
                              )}

                              {presupuesto.estado === 'enviado' && (
                                <DropdownMenuItem onClick={() => handlePresupuestoAction(presupuesto._id, 'recibido')}>
                                  <FileText className="mr-2 h-4 w-4 text-cyan-600" /> Marcar Recibido
                                </DropdownMenuItem>
                              )}
                              ...
                              {presupuesto.estado === 'recibido' && (
                                <>
                                  <DropdownMenuItem onClick={() => handlePresupuestoAction(presupuesto._id, 'aceptar')}>
                                    <CheckCircle2 className="mr-2 h-4 w-4 text-green-600" /> Aceptar
                                  </DropdownMenuItem>
                              ...
                              {presupuesto.estado === 'aceptado' && (
                                <DropdownMenuItem onClick={() => handlePresupuestoAction(presupuesto._id, 'convertir')}>
                                  <ShoppingCart className="mr-2 h-4 w-4 text-purple-600" /> Convertir a Pedido
                                </DropdownMenuItem>
                              )}

                              {presupuesto.estado !== 'cancelado' && presupuesto.estado !== 'convertido' && (
                                <DropdownMenuItem onClick={() => handlePresupuestoAction(presupuesto._id, 'cancelar')}>
                                  <XCircle className="mr-2 h-4 w-4 text-slate-600" /> Cancelar
                                </DropdownMenuItem>
                              )}
...
{canDelete('presupuestos-compra') && (
                              <>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-destructive" onClick={() => handlePresupuestoAction(presupuesto._id, 'delete')}>
        ...
        {pagination.total > 0 && (
          <Card className="p-4">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="sm" onClick={() => setFilters((prev: any) => ({ ...prev, page: 1 }))} disabled={pagination.page === 1} className="h-9 w-9 p-0" title="Primera pagina">
                  <ChevronsLeft className="h-4 w-4" />
                </Button>
                <Button variant="outline" size="sm" onClick={() => setFilters((prev: any) => ({ ...prev, page: Math.max(1, prev.page! - 1) }))} disabled={pagination.page === 1} className="h-9 w-9 p-0" title="Pagina anterior">
                ...
                {getPageNumbers(pagination.page, pagination.pages).map((pageNum, idx) => {
                  ...
                  return (
                    <Button key={page} variant={pagination.page === page ? 'default' : 'outline'} size="sm" onClick={() => setFilters((prev: any) => ({ ...prev, page }))} className="h-9 w-9 p-0">
                      {page}
                    </Button>
                  )
                })}

                <Button variant="outline" size="sm" onClick={() => setFilters((prev: any) => ({ ...prev, page: Math.min(pagination.pages, prev.page! + 1) }))} disabled={pagination.page === pagination.pages} className="h-9 w-9 p-0" title="Pagina siguiente">
                  <ChevronRight className="h-4 w-4" />
                </Button>
                <Button variant="outline" size="sm" onClick={() => setFilters((prev: any) => ({ ...prev, page: pagination.pages }))} disabled={pagination.page === pagination.pages} className="h-9 w-9 p-0" title="Ultima pagina">
        ...
        <Dialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ ...deleteDialog, open })}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Estas seguro de que deseas eliminar {deleteDialog.presupuestoIds.length === 1
                  ? 'el siguiente presupuesto de compra'
                  : `los siguientes ${deleteDialog.presupuestoIds.length} presupuestos de compra`}?
              </DialogDescription>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setDeleteDialog({ open: false, presupuestoIds: [], presupuestoCodigos: [] })}>
                Cancelar
              </Button>
