...
export default function PedidosCompraPage() {
  ...
  const handleBulkEmail = async () => {
    if (selectedPedidos.length === 0) {
      toast.error('Selecciona al menos un pedido para enviar por email')
      return
    }
    ...
    try {
      ...
      const response = await pedidosCompraService.enviarMasivoPorEmail(selectedPedidos)
  ...
  const handleBulkWhatsApp = async () => {
    if (selectedPedidos.length === 0) {
      toast.error('Selecciona al menos un pedido para enviar por WhatsApp')
      return
    }
    ...
    try {
      const result = await pedidosCompraService.getWhatsAppURLsMasivo(selectedPedidos)
  ...
  const handleBulkPrint = () => {
    ...
    // Abrir ventana de impresion para cada pedido seleccionado
    selectedPedidos.forEach((id, index) => {
      setTimeout(() => {
        window.open(`/compras/pedidos/${id}/imprimir`, '_blank', 'width=900,height=700')
      }, index * 500)
    })
  }
  ...
  const handlePedidoAction = async (pedidoId: string, action: string) => {
    switch (action) {
      ...
      case 'cancelar':
        ...
        break
      case 'crear-albaran':
        router.push(`/compras/pedidos/${pedidoId}/recepcion`)
        break
      default:
            ...
            <div className="py-4">
              ...
              {!isLoadingWhatsApp && whatsAppUrls.length > 0 && (
                <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                  <p className="text-xs text-muted-foreground">
                    <span className="font-medium text-green-600">{whatsAppUrls.filter(u => u.url).length}</span> de {whatsAppUrls.length} pedidos tienen telefono valido para WhatsApp
                  </p>
                </div>
              )}
            </div>
