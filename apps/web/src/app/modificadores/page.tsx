...
const TIPOS_MODIFICADOR: Record<string, { label: string; color: string }> = {
  gratis: { label: 'Gratis', color: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' },
  cargo: { label: 'Cargo extra', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' },
  descuento: { label: 'Descuento', color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' },
}
...
const DEFAULT_CONFIG = {
  columnas: [
    ...
    { key: 'precioExtra', visible: true, orden: 3 },
...
export default function ModificadoresPage() {
  ...
  const [modificadores, setModificadores] = useState<ModificadorProducto[]>([])
  ...
  const [columnasDisponibles] = useState([
    ...
    { key: 'precioExtra', label: 'Precio Extra', sortable: true },
  ...
  const {
    ...
    updateDensidad,
    resetConfig,
  } = useModuleConfig('modificadores', DEFAULT_CONFIG, { autoSave: true, debounceMs: 1000 })
  ...
  const stats = useMemo(() => {
    if (!modificadores?.length) return { total: 0, activos: 0, inactivos: 0 }
    return {
      total: pagination?.total || 0,
      activos: modificadores.filter(m => m?.activo).length,
      inactivos: modificadores.filter(m => !m?.activo).length,
    }
  }, [modificadores, pagination?.total])
  ...
  const cargarModificadores = useCallback(async () => {
    try {
      ...
      const response = await modificadoresService.getAll(params)
      if (response.success) {
        ...
        setModificadores([])
        toast.error('Error al cargar los modificadores')
      }
    } catch (error) {
      setModificadores([])
      toast.error('Error al cargar los modificadores')
  ...
  useEffect(() => { cargarModificadores() }, [cargarModificadores])
  ...
  useEffect(() => {
    ...
    if (debouncedColumnFilters.codigo) combinedFilters.codigo = debouncedColumnFilters.codigo
  ...
  const handleSelectAll = () => {
    setSelectedItems(selectAll ? [] : modificadores.map(m => m._id))
  ...
  const handleDeleteConfirm = async () => {
    try {
      await Promise.all(deleteDialog.ids.map(id => modificadoresService.delete(id)))
      toast.success('Modificador(es) eliminado(s) correctamente')
      cargarModificadores()
      setSelectedItems([])
      setSelectAll(false)
  ...
  const handleBulkDelete = () => {
    const nombres = modificadores.filter(m => selectedItems.includes(m._id)).map(m => m.nombre)
      ...
      <div className="w-full space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Gestiona los modificadores de productos</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" size="sm" onClick={() => setShowStats(!showStats)}>
              {showStats ? <Eye className="h-4 w-4" /> : <SlidersHorizontal className="h-4 w-4" />}
              <span className="ml-2 hidden sm:inline">Estadísticas</span>
            </Button>
            <Button variant="outline" size="sm" onClick={cargarModificadores}>
              <RefreshCw className="h-4 w-4" />
              <span className="ml-2 hidden sm:inline">Actualizar</span>
            </Button>
            <Button asChild size="sm">
              <Link href="/modificadores/nuevo">
                <Plus className="h-4 w-4 mr-2" />
                <span className="hidden sm:inline">Nuevo Modificador</span>
              </Link>
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            ...
            <ExportButton data={modificadores} columns={columnasDisponibles.filter((col) => columnasVisibles.includes(col.key)).map((col) => ({ key: col.key, label: col.label }))} filename="modificadores" stats={[{ label: 'Total', value: stats.total }]} />
            <PrintButton data={modificadores} columns={columnasDisponibles.filter((col) => columnasVisibles.includes(col.key)).map((col) => ({ key: col.key, label: col.label }))} title="Modificadores" stats={[{ label: 'Total', value: stats.total }]} filters={columnFiltersInput} />
          </div>
        </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  {columnasVisibles.includes('precioExtra') && (
                    <th className={`${densityClasses.header} text-right w-[120px]`}>
                      <span className="text-xs font-semibold uppercase tracking-wider">Precio Extra</span>
                    </th>
                  )}
                  ...
                  {columnasVisibles.includes('tipo') && (
                    <th className="px-3 py-1.5">
                      <TableSelect
                        ...
                        placeholder="Todos"
                        options={[{ value: 'all', label: 'Todos' }, ...Object.entries(TIPOS_MODIFICADOR).map(([value, { label }]) => ({ value, label }))]}
                      />
                    </th>
                  )}
                  {columnasVisibles.includes('precioExtra') && <th className="px-3 py-1.5"></th>}
                  {columnasVisibles.includes('activo') && (
                    <th className="px-3 py-1.5">
                      <TableSelect
              ...
              <tbody className="divide-y">
                {isLoading ? (
                  <tr><td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                    <RefreshCw className="h-6 w-6 animate-spin mx-auto mb-2" />Cargando modificadores...
                  </td></tr>
                ) : modificadores.length === 0 ? (
                  <tr><td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                    <SlidersHorizontal className="h-12 w-12 mx-auto mb-2 opacity-20" />
                    <p className="font-medium">No se encontraron modificadores</p>
                  </td></tr>
                ) : (
                  modificadores.map((mod) => (
                    <tr key={mod._id} className="hover:bg-muted/30 transition-colors group cursor-pointer" onClick={() => router.push(`/modificadores/${mod._id}`)}>
                      <td className={`${densityClasses.cell} sticky left-0 z-20 bg-card group-hover:bg-muted/50 transition-colors`} onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-center">
                          <Checkbox checked={selectedItems.includes(mod._id)} onCheckedChange={() => handleSelect(mod._id)} />
                        </div>
                      </td>

                      {columnasVisibles.includes('codigo') && (
                        <td className={`${densityClasses.cell} ${densityClasses.text} font-mono`}>{mod.codigo || '-'}</td>
                      )}

                      {columnasVisibles.includes('nombre') && (
                        <td className={`${densityClasses.cell} ${densityClasses.text} font-medium`}>{mod.nombre}</td>
                      )}

                      {columnasVisibles.includes('tipo') && (
                        <td className={`${densityClasses.cell}`}>
                          <Badge className={`text-xs font-medium ${TIPOS_MODIFICADOR[mod.tipo]?.color || 'bg-gray-100'}`}>
                            {TIPOS_MODIFICADOR[mod.tipo]?.label || mod.tipo}
                          </Badge>
                        </td>
                      )}

                      {columnasVisibles.includes('precioExtra') && (
                        <td className={`${densityClasses.cell} ${densityClasses.text} text-right font-mono`}>
                      ...
                      {columnasVisibles.includes('activo') && (
                        <td className={`${densityClasses.cell}`}>
                          <Badge variant={mod.activo ? 'default' : 'secondary'} className={`text-xs font-medium ${mod.activo ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400'}`}>
        ...
        <Dialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ ...deleteDialog, open })}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ¿Estás seguro de que deseas eliminar {deleteDialog.ids.length === 1 ? 'el siguiente modificador' : `los siguientes ${deleteDialog.ids.length} modificadores`}?
