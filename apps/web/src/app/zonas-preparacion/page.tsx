...
const DEFAULT_CONFIG = {
  columnas: [
    ...
    { key: 'tiempoPreparacion', visible: true, orden: 3 },
...
export default function ZonasPreparacionPage() {
  ...
  const [columnasDisponibles] = useState([
    ...
    { key: 'tiempoPreparacion', label: 'Tiempo (min)', sortable: true },
  ...
  const {
    ...
    updateDensidad,
    resetConfig,
  } = useModuleConfig('zonas-preparacion', DEFAULT_CONFIG, { autoSave: true, debounceMs: 1000 })
  ...
  const stats = useMemo(() => {
    if (!zonas?.length) return { total: 0, activos: 0, inactivos: 0 }
    return {
      total: pagination?.total || 0,
      activos: zonas.filter(e => e?.activo).length,
      inactivos: zonas.filter(e => !e?.activo).length,
    }
  }, [zonas, pagination?.total])
  ...
  const handleSelectAll = () => {
    setSelectedItems(selectAll ? [] : zonas.map(e => e._id))
  ...
  const handleDeleteConfirm = async () => {
    try {
      await Promise.all(deleteDialog.ids.map(id => zonasPreparacionService.delete(id)))
      toast.success('Zona(s) eliminada(s) correctamente')
      cargarZonas()
      setSelectedItems([])
      setSelectAll(false)
      ...
      <div className="w-full space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Gestiona las zonas de preparación (cocina, barra, etc.)</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" size="sm" onClick={() => setShowStats(!showStats)}>
              {showStats ? <Eye className="h-4 w-4" /> : <ChefHat className="h-4 w-4" />}
              <span className="ml-2 hidden sm:inline">Estadísticas</span>
            </Button>
            <Button variant="outline" size="sm" onClick={cargarZonas}>
              <RefreshCw className="h-4 w-4" />
              <span className="ml-2 hidden sm:inline">Actualizar</span>
            </Button>
            <Button asChild size="sm">
              <Link href="/zonas-preparacion/nuevo">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            ...
            <ExportButton data={zonas} columns={columnasDisponibles.filter((col) => columnasVisibles.includes(col.key)).map((col) => ({ key: col.key, label: col.label }))} filename="zonas-preparacion" stats={[{ label: 'Total', value: stats.total }]} />
            <PrintButton data={zonas} columns={columnasDisponibles.filter((col) => columnasVisibles.includes(col.key)).map((col) => ({ key: col.key, label: col.label }))} title="Zonas de Preparación" stats={[{ label: 'Total', value: stats.total }]} filters={columnFiltersInput} />
          </div>
        </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  {columnasVisibles.includes('tiempoPreparacion') && (
                    <th className={`${densityClasses.header} text-left w-[100px]`}>
                      <span className="text-xs font-semibold uppercase tracking-wider">Tiempo</span>
                    </th>
                  )}
                ...
                <tr className="border-b bg-card">
                  ...
                  {columnasVisibles.includes('codigo') && <th className="px-3 py-1.5"></th>}
                  {columnasVisibles.includes('color') && <th className="px-3 py-1.5"></th>}
                  {columnasVisibles.includes('tiempoPreparacion') && <th className="px-3 py-1.5"></th>}
                  {columnasVisibles.includes('activo') && (
                    <th className="px-3 py-1.5">
                      <TableSelect
              ...
              <tbody className="divide-y">
                {isLoading ? (
                  <tr><td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                    <RefreshCw className="h-6 w-6 animate-spin mx-auto mb-2" />Cargando zonas...
                  </td></tr>
                ) : zonas.length === 0 ? (
                  <tr><td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center text-sm text-muted-foreground">
                  ...
                  zonas.map((zona) => (
                    <tr key={zona._id} className="hover:bg-muted/30 transition-colors group">
                      <td className={`${densityClasses.cell} sticky left-0 z-20 bg-card group-hover:bg-muted/50 transition-colors`}>
                        <div className="flex items-center justify-center">
                          <Checkbox checked={selectedItems.includes(zona._id)} onCheckedChange={() => handleSelect(zona._id)} />
                        </div>
                      </td>

                      {columnasVisibles.includes('nombre') && (
                        <td className={`${densityClasses.cell} ${densityClasses.text} font-medium`}>{zona.nombre}</td>
                      )}

                      {columnasVisibles.includes('codigo') && (
                        <td className={`${densityClasses.cell} ${densityClasses.text} font-mono`}>{zona.codigo || '-'}</td>
                      )}

                      {columnasVisibles.includes('color') && (
                        <td className={`${densityClasses.cell}`}>
                          <div className="h-6 w-6 rounded-full border" style={{ backgroundColor: zona.color }} title={zona.color} />
                        </td>
                      )}

                      {columnasVisibles.includes('tiempoPreparacion') && (
                        <td className={`${densityClasses.cell} ${densityClasses.text}`}>
                          {zona.tiempoPreparacionPromedio ? `${zona.tiempoPreparacionPromedio} min` : '-'}
                        </td>
                      )}

                      {columnasVisibles.includes('activo') && (
                        <td className={`${densityClasses.cell}`}>
                          <Badge variant={zona.activo ? 'default' : 'secondary'} className={`text-xs font-medium ${zona.activo ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400'}`}>
        ...
        <Dialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ ...deleteDialog, open })}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ¿Estás seguro de que deseas eliminar {deleteDialog.ids.length === 1 ? 'la siguiente zona' : `las siguientes ${deleteDialog.ids.length} zonas`}?
