...
import { AlbaranCompraPrintView, defaultPrintOptions } from '@/components/compras/AlbaranCompraPrintView'
...
export default function AlbaranCompraImprimirPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [albaranRes, empresaRes] = await Promise.all([
          albaranesCompraService.getById(resolvedParams.id),
          empresaService.getMiEmpresa(),
        ])

        if (albaranRes.success && albaranRes.data) {
          setAlbaran(albaranRes.data)
        }
  ...
  if (!albaran) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-center">
          <p className="text-red-600 font-medium">Albaran de compra no encontrado</p>
          <button
            onClick={handleClose}
      ...
      <div className="print:hidden sticky top-0 z-50 bg-gray-800 text-white shadow-lg">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 rounded font-medium transition-colors"
            >
