...
import { FacturaCompraPrintView, defaultPrintOptions } from '@/components/compras/FacturaCompraPrintView'
...
export default function FacturaCompraImprimirPage({ params }: PageProps) {
    ...
    const loadData = async () => {
      try {
        const [facturaRes, empresaRes] = await Promise.all([
          facturasCompraService.getById(resolvedParams.id),
  ...
  if (!factura) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-center">
          <p className="text-red-600 font-medium">Factura de compra no encontrada</p>
          <button
            onClick={handleClose}
