...
import { TipoGasto, CATEGORIAS_TIPO_GASTO } from '@/types/tipo-gasto.types'
...
import {
  ...
  CheckCircle, XCircle, Copy, Receipt,
} from 'lucide-react'
...
interface TiposGastoFilters {
  page?: number; limit?: number; sortBy?: string; sortOrder?: 'asc' | 'desc'
  q?: string; activo?: boolean; categoria?: string; facturable?: string
}
...
const DEFAULT_CONFIG = {
  columnas: [
    ...
    { key: 'categoria', visible: true, orden: 2 },
    { key: 'ivaPorDefecto', visible: true, orden: 3 },
    { key: 'facturable', visible: true, orden: 4 },
...
export default function TiposGastoPage() {
  ...
  const [filters, setFilters] = useState<TiposGastoFilters>({ page: 1, limit: 25, sortBy: 'orden', sortOrder: 'asc', activo: true })
  ...
  const columnasDisponibles = [
    ...
    { key: 'ivaPorDefecto', label: 'IVA', sortable: true },
    { key: 'facturable', label: 'Facturable', sortable: true },
    { key: 'activo', label: 'Estado', sortable: true },
  ]

  const { config: moduleConfig, isLoading: isLoadingConfig, updateColumnas, updateSortConfig, updateDensidad, resetConfig } = useModuleConfig('tipos-gasto', DEFAULT_CONFIG, { autoSave: true, debounceMs: 1000 })
  ...
  const stats = useMemo(() => ({
    ...
    activos: tiposGasto.filter(f => f?.activo).length,
    inactivos: tiposGasto.filter(f => !f?.activo).length,
  }), [tiposGasto, pagination?.total])
  ...
  const cargarTiposGasto = useCallback(async () => {
    try {
      ...
      if (filters.facturable) params.facturable = filters.facturable
      const response = await tiposGastoService.getAll(params)
      if (response.data) {
  ...
  useEffect(() => { cargarTiposGasto() }, [cargarTiposGasto])
  ...
  useEffect(() => {
    ...
    if (debouncedColumnFilters.categoria && debouncedColumnFilters.categoria !== 'all') combinedFilters.categoria = debouncedColumnFilters.categoria
    if (debouncedColumnFilters.facturable && debouncedColumnFilters.facturable !== 'all') combinedFilters.facturable = debouncedColumnFilters.facturable
  ...
  const handleGuardarVista = useCallback(async (nombre: string, descripcion?: string, esDefault?: boolean, vistaId?: string) => {
    try {
      if (vistaId) {
        await vistasService.update(vistaId, { modulo: 'tipos-gasto', nombre, descripcion, configuracion: moduleConfig, esDefault: esDefault || false })
        toast.success(`Vista "${nombre}" actualizada`)
      } else {
        await vistasService.create({ modulo: 'tipos-gasto', nombre, descripcion, configuracion: moduleConfig, esDefault: esDefault || false })
  ...
  useEffect(() => {
    const cargarVistaDefault = async () => {
      try {
        const vistas = await vistasService.getAll('tipos-gasto', true)
  ...
  const handleSelectAll = () => { if (selectAll) setSelectedItems([]); else setSelectedItems(tiposGasto.map(f => f._id)); setSelectAll(!selectAll) }
  ...
  const handleDeleteConfirm = async () => { try { await tiposGastoService.deleteMany(deleteDialog.ids); toast.success('Tipo(s) de gasto eliminado(s)'); cargarTiposGasto(); setSelectedItems([]); setSelectAll(false); setDeleteDialog({ open: false, ids: [], nombres: [] }) } catch { toast.error('Error al eliminar') } }
  const handleDuplicar = async (id: string) => {
    try {
      const response = await tiposGastoService.duplicar(id)
      if (response.success && response.data) {
        toast.success('Tipo de gasto duplicado correctamente')
        router.push(`/tipos-gasto/${response.data._id}/editar`)
      }
    } catch (error: any) {
  ...
  const handleAction = async (id: string, action: string) => {
    switch (action) {
      case 'edit': router.push(`/tipos-gasto/${id}/editar`); break
      case 'duplicate': handleDuplicar(id); break
      case 'delete': const f = tiposGasto.find(fp => fp._id === id); if (f) setDeleteDialog({ open: true, ids: [id], nombres: [f.nombre] }); break
    }
  }
  const handleBulkAction = (action: string) => {
    if (action === 'delete' && selectedItems.length > 0) {
      const nombres = tiposGasto.filter(f => selectedItems.includes(f._id)).map(f => f.nombre)
  ...
  return (
      ...
      <div className="w-full space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2"><Receipt className="h-7 w-7 text-primary" />Tipos de Gasto</h1>
            <p className="text-sm text-muted-foreground mt-1">Catalogo de tipos de gastos imputables a partes de trabajo</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" size="sm" onClick={cargarTiposGasto}><RefreshCw className="h-4 w-4" /><span className="ml-2 hidden sm:inline">Actualizar</span></Button>
            {canCreate('tipos-gasto') && (
              <Button asChild size="sm"><Link href="/tipos-gasto/nuevo"><Plus className="h-4 w-4 mr-2" /><span className="hidden sm:inline">Nuevo Tipo</span></Link></Button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            <SettingsMenu densidad={densidad} onDensidadChange={(d) => { updateDensidad(d); toast.success(`Densidad: ${d}`) }} modulo="tipos-gasto" configuracionActual={moduleConfig} onAplicarVista={handleAplicarVista} onGuardarVista={handleGuardarVista} onRestablecer={async () => { await resetConfig(); toast.success('Configuracion restablecida') }} />
            ...
            <ExportButton data={tiposGasto} columns={columnasDisponibles.filter(c => columnasVisibles.includes(c.key)).map(c => ({ key: c.key, label: c.label }))} filename="tipos-gasto" stats={[{ label: 'Total', value: stats.total }, { label: 'Activos', value: stats.activos }]} />
            <PrintButton data={tiposGasto} columns={columnasDisponibles.filter(c => columnasVisibles.includes(c.key)).map(c => ({ key: c.key, label: c.label }))} title="Listado de Tipos de Gasto" stats={[{ label: 'Total', value: stats.total }, { label: 'Activos', value: stats.activos }]} filters={columnFiltersInput} />
          </div>
        </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  {columnasVisibles.includes('categoria') && <th className={`${densityClasses.header} text-left w-[150px]`}><button onClick={() => handleSort('categoria')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider">Categoria{getSortIcon('categoria')}</button></th>}
                  {columnasVisibles.includes('ivaPorDefecto') && <th className={`${densityClasses.header} text-left w-[80px]`}><button onClick={() => handleSort('ivaPorDefecto')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider">IVA{getSortIcon('ivaPorDefecto')}</button></th>}
                  {columnasVisibles.includes('facturable') && <th className={`${densityClasses.header} text-left w-[100px]`}><button onClick={() => handleSort('facturable')} className="flex items-center hover:text-primary text-xs font-semibold uppercase tracking-wider">Facturable{getSortIcon('facturable')}</button></th>}
                ...
                <tr className="border-b bg-card">
                  ...
                  {columnasVisibles.includes('categoria') && <th className="px-3 py-1.5"><TableSelect value={columnFiltersInput.categoria || 'all'} onValueChange={(v) => handleColumnFilterInput('categoria', v)} placeholder="Todos" options={[{ value: 'all', label: 'Todos' }, ...CATEGORIAS_TIPO_GASTO]} /></th>}
                  {columnasVisibles.includes('ivaPorDefecto') && <th className="px-3 py-1.5"></th>}
                  {columnasVisibles.includes('facturable') && <th className="px-3 py-1.5"><TableSelect value={columnFiltersInput.facturable || 'all'} onValueChange={(v) => handleColumnFilterInput('facturable', v)} placeholder="Todos" options={[{ value: 'all', label: 'Todos' }, { value: 'true', label: 'Si' }, { value: 'false', label: 'No' }]} /></th>}
              ...
              <tbody className="divide-y">
                {isLoading ? (
                  ...
                  <tr><td colSpan={columnasVisibles.length + 2} className="px-3 py-12 text-center"><Receipt className="h-12 w-12 mx-auto mb-2 opacity-20" /><p className="font-medium">No se encontraron tipos de gasto</p></td></tr>
                ) : tiposGasto.map((tg) => (
                  <tr key={tg._id} className="hover:bg-muted/30 transition-colors group">
                    <td className={`${densityClasses.cell} sticky left-0 z-20 bg-card group-hover:bg-muted/50`}><Checkbox checked={selectedItems.includes(tg._id)} onCheckedChange={() => handleSelectItem(tg._id)} /></td>
                    {columnasVisibles.includes('codigo') && <td className={`${densityClasses.cell} font-mono font-medium`}>{tg.codigo}</td>}
                    {columnasVisibles.includes('nombre') && <td className={`${densityClasses.cell} font-medium`}>{tg.nombre}</td>}
                    {columnasVisibles.includes('categoria') && <td className={`${densityClasses.cell}`}><Badge variant="outline" className="font-normal">{CATEGORIAS_TIPO_GASTO.find(c => c.value === tg.categoria)?.label || tg.categoria}</Badge></td>}
                    {columnasVisibles.includes('ivaPorDefecto') && <td className={`${densityClasses.cell} font-mono`}>{tg.ivaPorDefecto}%</td>}
                    {columnasVisibles.includes('facturable') && <td className={`${densityClasses.cell}`}>{tg.facturable ? <CheckCircle className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-muted-foreground" />}</td>}
                    {columnasVisibles.includes('activo') && <td className={`${densityClasses.cell}`}><Badge variant={tg.activo ? 'default' : 'secondary'} className={tg.activo ? 'bg-green-100 text-green-800' : ''}>{tg.activo ? 'Activo' : 'Inactivo'}</Badge></td>}
                    <td className={`${densityClasses.cell} sticky right-0 z-20 bg-card group-hover:bg-muted/50 text-right`}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56">
                          ...
                          <DropdownMenuItem onClick={() => handleAction(tg._id, 'edit')}><Edit className="mr-2 h-4 w-4" />Editar</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleAction(tg._id, 'duplicate')}><Copy className="mr-2 h-4 w-4" />Duplicar</DropdownMenuItem>
                          {canDelete('tipos-gasto') && (
                            <>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-destructive" onClick={() => handleAction(tg._id, 'delete')}><Trash2 className="mr-2 h-4 w-4" />Eliminar</DropdownMenuItem>
                            </>
                          )}
                        </DropdownMenuContent>
                      </DropdownMenu>
        ...
        <Dialog open={deleteDialog.open} onOpenChange={(o) => setDeleteDialog({ ...deleteDialog, open: o })}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>¿Eliminar {deleteDialog.ids.length === 1 ? 'el tipo de gasto' : `${deleteDialog.ids.length} tipos de gasto`}?</DialogDescription>
