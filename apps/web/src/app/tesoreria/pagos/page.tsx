...
export default function PagosPage() {
  ...
  const handleRegistrarPago = async () => {
    if (!pagoDialog.vencimiento || !importePago) return
    ...
    try {
      ...
      setPagoDialog({ open: false, vencimiento: null })
      setImportePago('')
      setReferenciaPago('')
      cargarVencimientos()
    } catch (error: any) {
      ...
      <div className="w-full space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Gestión de vencimientos a proveedores
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={() => router.push('/tesoreria/pagos/nuevo')}>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative flex-1 w-full sm:max-w-md">
            ...
            <Input
              placeholder="Buscar por proveedor, número, factura..."
              value={searchTerm}
                ...
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="px-4 py-3 text-left">
                    <button onClick={() => handleSort('terceroNombre')} className="flex items-center text-xs font-semibold uppercase">
                      Proveedor {getSortIcon('terceroNombre')}
                    </button>
                  </th>
                ...
                {isLoading ? (
                  ...
                  vencimientos.map((venc) => (
                    <tr key={venc._id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-bold text-red-600">{formatCurrency(venc.importePendiente)}</td>
                      <td className="px-4 py-3">
                            ...
                            {(venc.estado === 'pendiente' || venc.estado === 'parcial') && (
                              <>
                                ...
                                <DropdownMenuItem onClick={() => {
                                  setPagoDialog({ open: true, vencimiento: venc })
        ...
        <Dialog open={pagoDialog.open} onOpenChange={(open) => setPagoDialog({ open, vencimiento: null })}>
          <DialogContent>
            <DialogHeader>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setPagoDialog({ open: false, vencimiento: null })}>
                Cancelar
              </Button>
