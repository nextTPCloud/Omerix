...
interface FacturasAlertasProps {
  diasAlerta?: number;
  onRefresh?: () => void;
  className?: string;
  collapsible?: boolean;
...
export function FacturasAlertas({
  diasAlerta = 7,
  onRefresh,
  className,
  collapsible = true,
  defaultCollapsed = false,
}: FacturasAlertasProps) {
  ...
  const fetchAlertas = async () => {
    try {
      setLoading(true);
      const response = await facturasService.getAlertas(diasAlerta);
  ...
  const AlertaItem = ({ item, tipo }: { item: IFactura; tipo: 'pendiente' | 'vencida' | 'proxima' }) => {
    const fechaVenc = item.fechaVencimiento ? String(item.fechaVencimiento) : undefined;
    const diasInfo = getDiasVencimiento(fechaVenc);
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {item.importePendiente > 0 && (
              <span className="text-xs text-muted-foreground">
                Pte: {formatCurrency(item.importePendiente)}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <TabsList className="h-8">
          <TabsTrigger value="pendientes" className="text-xs px-3 h-7 gap-1.5">
            ...
            Ptes. cobro
            {resumen && resumen.pendientesCobro > 0 && (
              <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-xs">
                {resumen.pendientesCobro}
              </Badge>
            )}
          </TabsTrigger>
      ...
      {loading ? (
        ...
        <>
          <TabsContent value="pendientes" className="mt-0">
            {alertas?.pendientesCobro && alertas.pendientesCobro.length > 0 ? (
              <div className="space-y-1 max-h-[300px] overflow-y-auto">
                {alertas.pendientesCobro.map((item) => (
            ...
            {alertas?.vencidas && alertas.vencidas.length > 0 ? (
              <div className="space-y-1 max-h-[300px] overflow-y-auto">
                {alertas.vencidas.map((item) => (
                  <AlertaItem key={item._id} item={item} tipo="vencida" />
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-sm text-muted-foreground">
            ...
            {alertas?.proximasVencer && alertas.proximasVencer.length > 0 ? (
              <div className="space-y-1 max-h-[300px] overflow-y-auto">
                {alertas.proximasVencer.map((item) => (
                  <AlertaItem key={item._id} item={item} tipo="proxima" />
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-sm text-muted-foreground">
  ...
  if (!collapsible) {
    return (
      <Card className={cn('border-purple-200 bg-purple-50/50 dark:bg-purple-950/20 dark:border-purple-900/50', className)}>
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className={className}>
      <Card className={cn(
        'border-purple-200 bg-purple-50/50 dark:bg-purple-950/20 dark:border-purple-900/50',
        !isOpen && 'cursor-pointer hover:bg-purple-100/50 dark:hover:bg-purple-950/30'
      )}>
...
export default FacturasAlertas;
