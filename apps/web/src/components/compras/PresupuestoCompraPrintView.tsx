...
interface PresupuestoCompraPrintViewProps {
...
/**
 * Componente de vista de impresion para presupuestos de compra.
 * Diseno profesional con cabecera de empresa, datos del proveedor y lineas.
 */
export const PresupuestoCompraPrintView = forwardRef<HTMLDivElement, PresupuestoCompraPrintViewProps>(
      ...
      >
        ...
        <header className="flex justify-between items-start border-b-2 border-gray-800 pb-4 mb-6">
          ...
          <div className="text-right">
            <h2 className="text-2xl font-bold text-gray-900">SOLICITUD DE PRESUPUESTO</h2>
            ...
            {presupuesto.fechaValidez && (
              <p className="text-sm text-gray-600">
                Valido hasta: {formatDate(presupuesto.fechaValidez)}
              </p>
            )}
          </div>
        </header>
        ...
        <section className="grid grid-cols-2 gap-8 mb-6">
          <div className="bg-gray-50 p-4 rounded border border-gray-200">
            ...
            <p className="text-gray-700">NIF: {presupuesto.proveedorNif}</p>
            {presupuesto.proveedorEmail && (
              <p className="text-gray-600 text-sm">{presupuesto.proveedorEmail}</p>
            )}
            {presupuesto.proveedorTelefono && (
              <p className="text-gray-600 text-sm">Tel: {presupuesto.proveedorTelefono}</p>
            )}
            {presupuesto.proveedorDireccion && (
          ...
          <div className="bg-gray-50 p-4 rounded border border-gray-200">
            ...
            {presupuesto.referenciaProveedor && (
              <p className="text-gray-600 text-sm">
                <span className="font-medium">Ref. Proveedor:</span> {presupuesto.referenciaProveedor}
              </p>
            )}
            {presupuesto.fechaSolicitud && (
              <p className="text-gray-600 text-sm">
                <span className="font-medium">Fecha Solicitud:</span> {formatDate(presupuesto.fechaSolicitud)}
              </p>
            )}
            {presupuesto.prioridad && (
              <p className="text-gray-600 text-sm">
                <span className="font-medium">Prioridad:</span> {presupuesto.prioridad}
              </p>
            )}
          </div>
        </section>
        ...
        <section className="flex justify-end mb-6">
          <div className="w-72 border border-gray-300 rounded overflow-hidden">
            ...
            {presupuesto.descuentoGlobalPorcentaje && presupuesto.descuentoGlobalPorcentaje > 0 && (
        ...
        <footer className="mt-8 pt-4 border-t border-gray-300">
          ...
          {presupuesto.observaciones && (
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-600 whitespace-pre-wrap">{presupuesto.observaciones}</p>
            </div>
          )}

          {/* Condiciones del proveedor */}
          {options.mostrarCondiciones && presupuesto.condicionesProveedor && (
            <div className="mb-4 text-sm text-gray-600 whitespace-pre-wrap bg-gray-50 p-3 rounded border border-gray-200">
              <h4 className="font-semibold text-gray-800 mb-2 text-sm">Condiciones del Proveedor:</h4>
...
PresupuestoCompraPrintView.displayName = 'PresupuestoCompraPrintView'

export default PresupuestoCompraPrintView
