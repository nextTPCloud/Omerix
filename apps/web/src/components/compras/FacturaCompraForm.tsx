...
export function FacturaCompraForm({
  ...
  // Handlers wrapper para DocumentoLineasGrid
  ...
  const handleDuplicateLinea = (index: number) => {
    const nuevasLineas = [...lineas]
    const lineaOriginal = nuevasLineas[index]
    const lineaNueva: LineaFormulario = {
      ...lineaOriginal,
      _id: undefined,
      orden: nuevasLineas.length + 1,
    }
    nuevasLineas.splice(index + 1, 0, lineaNueva)
    setLineas(nuevasLineas.map((l, i) => ({ ...l, orden: i + 1 })))
  }
    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="lineas" className="space-y-4">
          <DocumentoLineasGrid
            moduloNombre="facturas-compra-lineas"
