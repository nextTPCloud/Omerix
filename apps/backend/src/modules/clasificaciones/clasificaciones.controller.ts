...
export class ClasificacionesController {
  ...
  async duplicar(req: Request, res: Response) {
    try {
      ...
      const clasificacion = await clasificacionesService.duplicar(
        req.params.id,
        empresaId,
        usuarioId,
        req.empresaDbConfig
      ...
      res.status(201).json({
        success: true,
        data: clasificacion,
        message: 'Clasificación duplicada correctamente',
      });
    } catch (error: any) {
      console.error('Error al duplicar clasificación:', error);
      res.status(error.message === 'Clasificación no encontrada' ? 404 : 500).json({
        success: false,
        message: error.message || 'Error al duplicar la clasificación',
      });
    }
  }
}
